...
with lib; let
  ...
  mkSecretPath = mkName: subvol:
    if config.deployment.storeKeysOnMachine
    then "/etc/${config.environment.etc."keys/${mkName subvol}".target}"
    else config.deployment.keys.${mkName subvol}.path;
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      services.borgbackup.jobs =
        mapAttrs'
        (_: subvol:
          nameValuePair (mkJobName subvol) {
            inherit (subvol) repo;
