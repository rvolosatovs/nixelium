  ...
  lua_const_table = with pkgs.lib; attrs: ''
    setmetatable({
      ...
      __index = function(t, key)
        return t[key] or error("attempt to get "..quote(key))
      end,
      __newindex = function(_, key, value)
        error("attempt to set "..quote(key).." to "..quote(value))
      end,
    })
  '';
in ''
  ...
  bin = with pkgs; lua_const_table {
    bash-language-server = builtins.toJSON "${nodePackages.bash-language-server}/bin/bash-language-server";
    clangd = builtins.toJSON "${clang-tools}/bin/clangd";
    docker-langserver = builtins.toJSON "${nodePackages.dockerfile-language-server-nodejs}/bin/docker-langserver";
    elm = builtins.toJSON "${elmPackages.elm}/bin/elm";
    elm-format = builtins.toJSON "${elmPackages.elm-format}/bin/elm-format";
    elm-language-server = builtins.toJSON "${elmPackages.elm-language-server}/bin/elm-language-server";
