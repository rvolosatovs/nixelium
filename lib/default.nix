...
rec {
  luaConstTable =
    t: "setmetatable(${t},{__index=${luaConstTableIndex}, __newindex=${luaConstTableNewIndex}})";
  ...
  toLua =
    ...
    if isAttrs v then
      ...
      if length v == 0 then luaConstTable "{}" else throw "non-empty lists are not supported"
    else
      toJSON v;
